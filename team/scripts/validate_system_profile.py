...
def main() -> int:
    profile_path = os.path.join(REPO_ROOT, "team", "config", "system_profile.yaml")
    ...
    if not isinstance(profile, dict):
        print("FAIL profile must be a top-level mapping/object")
        return 1
    ...
    if errors:
        for err in errors[:5]:
            print(f"FAIL {_error_details(err)}")
    ...
    if isinstance(skills_dir, str):
        ...
        if not os.path.isdir(resolved):
            print(f"FAIL skills.directory does not exist: {resolved}")
            return 1
    print("System profile passed validation.")
    return 0
